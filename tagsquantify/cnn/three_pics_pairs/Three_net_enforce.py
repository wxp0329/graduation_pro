...
def inference1(images, batch=FLAGS.batch_size):
    ...
    # drop0 = tf.nn.dropout(local3, keep_prob=0.5)
    # # # local4
    # with tf.variable_scope('local4') as scope:
    #     weights = _variable_with_weight_decay('weights', shape=[384, 192],
    #                                           stddev=0.1, wd=0.004)
    #     biases = _variable_on_cpu('biases', [192], tf.constant_initializer(0.))
    #     local4 = tf.nn.relu(tf.matmul(drop0, weights) + biases, name=scope.name)
    #     _activation_summary(local4)

    drop = tf.nn.dropout(local3, keep_prob=0.5)
    # # # # affine
    with tf.variable_scope('affine') as scope:
        weights = _variable_with_weight_decay('weights', [1000, 32],
                                              stddev=0.1, wd=0.0)
...
def inference(images, batch=FLAGS.batch_size):
    ...
    # # # # affine
    with tf.variable_scope('affine') as scope:
        weights = _variable_with_weight_decay('weights', [192, 32],
                                              stddev=0.1, wd=0.0)
        biases = _variable_on_cpu('biases', [32],
                                  tf.constant_initializer(0.))
        affine = tf.nn.sigmoid(tf.matmul(drop, weights) + biases, name=scope.name)

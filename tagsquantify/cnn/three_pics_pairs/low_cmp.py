...
def cnn_gen_sim_pics(retrived_pics):
    ...
    with open('/media/wangxiaopeng/maxdisk/NUS_dataset/tags/tags_after/218838_key_list.txt') as fr:
        ...
        for i in fr.readlines():
            ...
            count += 1
    mat_2003 = np.load('/home/wangxiaopeng/NUS_dataset/enforce_mats_500/2003_hash_mat_48a.npy')
    mat_218838 = np.load('/home/wangxiaopeng/NUS_dataset/enforce_mats_500/combine_hash_pic_mat_48a.npy')
    # flat_218838 = []
    # for i in mat_218838:
    #     flat_218838.append(i.flatten())
    pics_root_dir = '/home/wangxiaopeng/all_find_pics/find_pic_cnn_' + retrived_pics.rsplit('_', 1)[1].split('.')[0]
    # if os.path.exists(pics_root_dir):
    #     shutil.rmtree(pics_root_dir)
    # os.mkdir(pics_root_dir)

    fw = open('/home/wangxiaopeng/lib_data_old/cnn_acc_map_recall_32a/cnn_topic_time.txt', 'a')
...
def cnn_acc_map_recall():
    root_dir = '/home/wangxiaopeng/lib_data_old/cnn_acc_map_recall_32a'

    shutil.rmtree(root_dir)
    os.mkdir(root_dir)

    file_paths = []
    for root, dirs, files in os.walk('/media/wangxiaopeng/maxdisk/NUS_dataset/2003_in_Allbels_index_5/'):
        for file in files:
            file_paths.append(os.path.join(root, file))
    test_2003_avg = []
    test_2003_map = []
    test_2003_recall = []
    fw = open(root_dir+'/all_2003_avg_acc.txt', 'w')
    fw_map = open(root_dir+'/all_2003_avg_map.txt', 'w')
    fw_recall = open(root_dir+'/all_2003_avg_recall.txt', 'w')

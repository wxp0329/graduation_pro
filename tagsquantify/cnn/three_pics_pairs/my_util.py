...
def get2003features():
    ...
    with open('/media/wangxiaopeng/maxdisk/NUS_dataset/tags/tags_after/218838_key_list.txt') as fr:
        for i in fr.readlines():
            file_names.append(os.path.join('/media/wangxiaopeng/maxdisk/NUS_dataset/images_220841', i.strip() + '.jpg'))
    pics = []
    num = 0
    for i in file_names:
        pics.append(getimg(i))
        num = num + 1
        print('operating :',num)
    np.save(open('/media/wangxiaopeng/maxdisk/NUS_dataset/218838_conv_pics.npy', 'w'), pics)
